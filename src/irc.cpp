...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #ttcoincoinTEST\r");
            Send(hSocket, "WHO #ttcoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #ttcoincoin\n");
            // randomly join #ttcoincoin00-#ttcoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ttcoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ttcoincoin%02d\r", channel_number).c_str());
        };
